...
export async function run() {
  ...
  // carries `@import "tailwindcss"` and the design tokens, and adds one
  // `@source` per class-bearing directory of every module (core + enabled
  // features). This is required because the packages live in `node_modules`,
  // which Tailwind never auto-scans; only the paths named here are considered.
  // Both components AND route files must be covered — some custom `@utility`
  // classes (e.g. `text-h1`) are used only in route pages, so sourcing only
  // `components/` would leave those utilities un-emitted. Paths are emitted
  // relative to the manifest so the output is machine-independent.
  ...
  // Directories that can contain Tailwind class names: JSX components and every
  // route source (`lib/` is logic, `styles/` is CSS, i18n JSON is copy — none
  // hold utility classes, so they are excluded to keep the scan tight).
  const UI_SOURCE_DIRS = ['components', 'routes', 'routes-app', 'routes-public', 'routes-root'];
  const sourceDirsFor = (id) =>
    UI_SOURCE_DIRS.map((d) => join(moduleRoot(id), d)).filter(existsSync);
  const css = [
    '/* GENERATED by `modelcity gen` — do not edit. Run `npm run gen:modules`. */',
    `@import "${cssRel(coreGlobals)}";`,
    ...[CORE_MODULE.id, ...enabled.map((m) => m.id)]
      .flatMap(sourceDirsFor)
      .map((dir) => `@source "${cssRel(dir)}";`),
    '',
  ].join('\n');
