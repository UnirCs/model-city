...
# Backend project location — Maven project root of the monolith (built and dockerized by the deploy script).
variable "backend_project_dir" {
  description = "Absolute path to the Spring Boot monolith backend project directory (Maven root)."
  type        = string
}
...
# Monolith — single logical database name on the shared RDS instance.
variable "monolith_db_name" {
  description = "Database the monolith connects to (single DB for all verticals)."
  type        = string
  default     = "modelcity"
}

# Monolith — Fargate task sizing. Bigger than a single microservice since it
# carries every vertical in one process.
variable "monolith_cpu" {
  description = "Fargate CPU units for the monolith task."
  type        = number
  default     = 1024
}

variable "monolith_memory" {
  description = "Fargate memory (MiB) for the monolith task."
  type        = number
  default     = 2048
}
