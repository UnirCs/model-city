...
# Target group: Spring Cloud Gateway (backend API + mTLS flows).
...
# Default: frontend. Rule priority 10: /api/* → gateway.
...
# Route /api/* to the backend gateway on port 443.
...
# All traffic goes directly to the gateway.
