...
class CoreTrailsUseCasesTest {

    @Mock
    CoreSystemTrailStore store;
    ...
    void execute_returnsPageOfTrails() {
        ...
        when(trail.getEventType()).thenReturn("USER_REGISTERED");
        when(trail.getResourceType()).thenReturn("USER");
        when(trail.getResourceId()).thenReturn("user-sub");
        ...
        assertThat(result.getContent().get(0).eventType()).isEqualTo("USER_REGISTERED");
    }
