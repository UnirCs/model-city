...
/** Default {@link GetReservableResourcesForEditUseCase} implementation. The component-scanned platform default; disabled at startup when a local deployment provides its own bean for the seam. */
@RequiredArgsConstructor
@Service
@ModelCityDisabledIfInherited
public class DefaultGetReservableResourcesForEditUseCase implements GetReservableResourcesForEditUseCase<ReservableResourceDto> {
    ...
    public Page<ReservableResourceDto> execute(Long publicSpaceId, Pageable pageable, String locale) {
        ...
        return reservableResourceStore.findActiveByPublicSpace(publicSpaceId, pageable)
                .map(r -> ReservableResourceDto.fromWithTranslations(r, locale));
    }
}
