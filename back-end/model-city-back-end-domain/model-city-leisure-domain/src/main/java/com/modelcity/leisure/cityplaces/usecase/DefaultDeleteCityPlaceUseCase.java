...
import org.springframework.stereotype.Service;import com.modelcity.leisure.cityplaces.controller.model.CityPlaceRequestDto;
...
/** Default {@link DeleteCityPlaceUseCase} implementation. The component-scanned platform default; disabled at startup when a local deployment provides its own bean for the seam. */
@Slf4j
@Transactional
@RequiredArgsConstructor
@Service
@ModelCityDisabledIfInherited
public class DefaultDeleteCityPlaceUseCase implements DeleteCityPlaceUseCase {
    ...
    public void execute(Long id, String sub) {
        CityPlaceView place = cityPlaceStore.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("CityPlace", id));
        cityPlaceStore.deleteById(id);
        systemEventGenerator.cityPlaceDeleted(sub, place);
        log.info("CityPlace id={} deleted by sub={}", id, sub);
    }
}
