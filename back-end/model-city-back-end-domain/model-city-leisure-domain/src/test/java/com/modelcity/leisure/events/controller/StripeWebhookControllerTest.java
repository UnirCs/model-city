...
class StripeWebhookControllerTest {
    ...
    void setUp() {
        ...
        controller = new DefaultStripeWebhookController(properties, eventTicketStore, systemTrailGenerator);
    }
    ...
    void handleWebhook_invalidSignature_returnsBadRequest() {
        ...
        verifyNoInteractions(eventTicketStore);
    }

    @Test
    void handleWebhook_checkoutSessionCompleted_marksTicketAsPaid() {
        EventTicketView ticket = mock(EventTicketView.class);
        when(ticket.getId()).thenReturn(10L);
        when(eventTicketStore.findByCheckoutSessionId("cs_test_123")).thenReturn(Optional.of(ticket));
        ...
        verify(eventTicketStore).markStatus(10L, TicketStatus.PAID);
        verify(systemTrailGenerator).eventTicketStatusChanged(eq(ticket), eq(TicketStatus.PAID), any());
    }

    @Test
    void handleWebhook_checkoutSessionExpired_marksTicketAsCancelled() {
        EventTicketView ticket = mock(EventTicketView.class);
        when(ticket.getId()).thenReturn(10L);
        when(eventTicketStore.findByCheckoutSessionId("cs_test_123")).thenReturn(Optional.of(ticket));
        ...
        verify(eventTicketStore).markStatus(10L, TicketStatus.CANCELLED);
    }

    @Test
    void handleWebhook_asyncPaymentFailed_marksTicketAsCancelled() {
        EventTicketView ticket = mock(EventTicketView.class);
        when(ticket.getId()).thenReturn(10L);
        when(eventTicketStore.findByCheckoutSessionId("cs_test_123")).thenReturn(Optional.of(ticket));

        String payload = eventPayload("checkout.session.async_payment_failed", "cs_test_123");
        String signature = signedHeader(payload);
    ...
    @Test
    void handleWebhook_noMatchingTicket_doesNotThrow() {
        when(eventTicketStore.findByCheckoutSessionId("cs_unknown")).thenReturn(Optional.empty());
        ...
        assertThat(response.getStatusCode().value()).isEqualTo(200);
        verify(eventTicketStore, never()).markStatus(any(), any());
    }
}
