...
class LeisureTrailsUseCasesTest {

    @Mock
    LeisureSystemTrailStore store;
    ...
    void execute_returnsPageOfTrails() {
        ...
        when(trail.getEventType()).thenReturn("CITY_PLACE_CREATED");
        when(trail.getResourceType()).thenReturn("CITY_PLACE");
        ...
        assertThat(result.getContent().get(0).eventType()).isEqualTo("CITY_PLACE_CREATED");
    }

    @Test
    @SuppressWarnings("unchecked")
    void execute_emptyResult_returnsEmptyPage() {
        Page<SystemTrailView> page = new PageImpl<>(List.of());
        doReturn(page).when(store).search(any(SystemTrailQuery.class), any(Pageable.class));
