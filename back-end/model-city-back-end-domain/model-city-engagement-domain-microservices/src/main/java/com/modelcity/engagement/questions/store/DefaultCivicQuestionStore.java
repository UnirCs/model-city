...
/**
 * Microservice JPA adapter for the civic-question persistence port: stores zone/neighbourhood as soft
 * references (no FK, the zones live in another service); the default {@link CivicQuestionStore} bean.
 */
@Component
...
public class DefaultCivicQuestionStore implements CivicQuestionStore<CivicQuestion, CivicQuestionRequestDto> {
    ...
    private void applyFields(CivicQuestion question, CivicQuestionRequestDto request) {
        ...
        question.setZoneId(request.getZoneId());
        question.setNeighbourhoodId(request.getNeighbourhoodId());
